  ...
  return (
    <div className="relative w-full">
      ...
      {isOpen && (
        <div 
          ref={dropdownRef}
          className="absolute z-10 w-full mt-1 bg-popover text-popover-foreground border border-gray-200 rounded-md shadow-md max-h-27 overflow-auto"
        >
          {filteredOptions.length > 0 ? (
            <ul className="py-1">
              {filteredOptions.map((option) => (
                <li
                  key={option.id}
                  className="flex px-3 py-2 cursor-pointer [&:hover>svg]:block transition-colors hover:bg-accent focus:text-accent-foreground"
                  onClick={() => handleOptionSelect(option)}
                >
                  <span className='flex-auto whitespace-nowrap overflow-hidden text-ellipsis'>{option.label}</span>
                  {onDelete && (
                    <X size={16}
                      className='md:hidden rounded-sm hover:bg-card cursor-pointer'
