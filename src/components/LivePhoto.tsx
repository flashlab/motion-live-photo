...
export function LivePhoto({
  ...
  useEffect(() => {
    ...
    if (url && videoUrl) void initializeLivePhotosKit();
  }, [livePhotoRef, url, videoUrl, stamp]);

  return (
    <div
      ref={livePhotoRef}
      style={{ aspectRatio: aspectRatio }}
      className={cn(className, "relative w-full object-contain")}
    >
      {!window.LivePhotosKit && (
        <Loader className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 animate-spin" />
      )}
    </div>
  );
}
